...
| 数据库凭据 | user=`jkapi`, password=`jkapi`, dbname=`jkapi` |
...
psql -U jkapi -h 127.0.0.1 -d jkapi -f temp.sql
```
...
3. 无密码登录并重置
   ...
   ALTER USER jkapi WITH PASSWORD 'jkapi';
...
```bash
# 连接数据库
psql -U jkapi -h 127.0.0.1 -d jkapi
...
# 执行 SQL 文件
psql -U jkapi -h 127.0.0.1 -d jkapi -f migration.sql
```
...
- [上游仓库](https://github.com/jksn12/JKAPI)
