...
func (p *darwinProvider) Generate(ctx context.Context) (string, error) {
	...
	command.Env = []string{
		"PATH=/usr/bin:/bin",
		"JKAPI_DEVICECHECK_MODULE=" + modulePath,
		"JKAPI_ATTESTATION_BUNDLE_ID=" + bundleID,
		"JKAPI_ATTESTATION_SIGNALS=" + string(signalsJSON),
	}
...
const addon = require(process.env.JKAPI_DEVICECHECK_MODULE);
const signals = JSON.parse(process.env.JKAPI_ATTESTATION_SIGNALS);
const bundleID = process.env.JKAPI_ATTESTATION_BUNDLE_ID;
