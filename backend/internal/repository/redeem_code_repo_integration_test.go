...
func (s *RedeemCodeRepoSuite) TestListWithFilters_Category() {
	s.Require().NoError(s.repo.Create(s.ctx, &service.RedeemCode{Code: "CAT-YUNMAO", Type: service.RedeemTypeBalance, Category: "云猫充值", Value: 0, Status: service.StatusUnused}))
	s.Require().NoError(s.repo.Create(s.ctx, &service.RedeemCode{Code: "CAT-SUB", Type: service.RedeemTypeSubscription, Category: "订阅卡密", Value: 0, Status: service.StatusUnused}))

	codes, _, err := s.repo.ListWithFilters(s.ctx, pagination.PaginationParams{Page: 1, PageSize: 10}, "", "", "", "云猫充值")
	s.Require().NoError(err)
	s.Require().Len(codes, 1)
	s.Require().Equal("CAT-YUNMAO", codes[0].Code)
	s.Require().Equal("云猫充值", codes[0].Category)
}
