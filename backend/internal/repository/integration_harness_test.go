...
func TestMain(m *testing.M) {
	...
	pgContainer, err := tcpostgres.Run(
		ctx,
		postgresImage,
		tcpostgres.WithDatabase("jkapi_test"),
...
//
// JKAPI_TEST_POSTGRES_IMAGE overrides the PostgreSQL image so the suite can be
...
func selectDockerImage(ctx context.Context, preferred string) string {
	if override := strings.TrimSpace(os.Getenv("JKAPI_TEST_POSTGRES_IMAGE")); override != "" &&
