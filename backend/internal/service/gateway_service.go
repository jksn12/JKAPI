...
# Tone and style
 ...
 - Do not use a colon before tool calls. Your tool calls may not be shown directly in the output, so text like "Let me read the file:" followed by a read tool call should just be "Let me read the file." with a period.`
	...
	debugGatewayBodyEnv                    = "JKAPI_DEBUG_GATEWAY_BODY"
...
type GatewayService struct {
	...
	debugGatewayBodyFile  atomic.Pointer[os.File] // non-nil when JKAPI_DEBUG_GATEWAY_BODY is set
...
func NewGatewayService(
	...
	svc.debugModelRouting.Store(parseDebugEnvBool(os.Getenv("JKAPI_DEBUG_MODEL_ROUTING")))
	svc.debugClaudeMimic.Store(parseDebugEnvBool(os.Getenv("JKAPI_DEBUG_CLAUDE_MIMIC")))
...
// 启用方式（环境变量）：
//
//	JKAPI_DEBUG_GATEWAY_BODY=1                          # 写入 gateway_debug.log
//	JKAPI_DEBUG_GATEWAY_BODY=/tmp/gateway_debug.log     # 写入指定路径
//
