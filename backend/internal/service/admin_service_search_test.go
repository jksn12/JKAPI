...
func TestAdminService_ListRedeemCodes_WithSearch(t *testing.T) {
	t.Run("search 参数正常传递到 repository 层", func(t *testing.T) {
		...
		codes, total, err := svc.ListRedeemCodes(context.Background(), 1, 20, RedeemTypeBalance, StatusUnused, "ABC", "云猫充值", "value", "ASC")
		require.NoError(t, err)
		...
		require.Equal(t, "云猫充值", repo.listWithFiltersCategory)
	})
}
