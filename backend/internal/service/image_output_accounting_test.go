...
func TestOpenAIImageOutputCounter_NestedJKAPI_StandardResponse(t *testing.T) {
	// Simulate what a nested jkapi setup receives
	// When the upstream jkapi returns a standard response, the downstream
	// jkapi receives SSE events. This test verifies that no false positives
	// occur.
...
data: [DONE]`
	...
	count := countOpenAIImageOutputsFromSSEBody(sseBody)
	if count != 0 {
		t.Errorf("expected 0 images for nested jkapi text-only response, got %d", count)
	}
}
