...
ENV_FILE="${JKAPI_ENV_FILE:-${SCRIPT_DIR}/.env}"

STACK_LABEL_KEY="org.jkapi.stack"
STACK_LABEL_VALUE="apple-container"
NETWORK_NAME="jkapi-apple"
APP_CONTAINER="jkapi-apple"
POSTGRES_CONTAINER="jkapi-apple-postgres"
REDIS_CONTAINER="jkapi-apple-redis"
APP_VOLUME="jkapi-apple-data"
POSTGRES_VOLUME="jkapi-apple-postgres-data"
REDIS_VOLUME="jkapi-apple-redis-data"
PLATFORM="linux/arm64"

TEMP_DIR=""
LOCK_DIR="${TMPDIR:-/tmp}/jkapi-apple-container.lock"
LOCK_ACQUIRED=false
...
Commands:
  ...
  up [--recreate]       Create and start the complete JKAPI stack
...
Environment:
  JKAPI_ENV_FILE      Path to the deployment env file (default: deploy/.env)
EOF
}
...
acquire_lock() {
    if ! mkdir "${LOCK_DIR}" 2>/dev/null; then
        ...
        if [[ "${owner_pid}" =~ ^[0-9]+$ ]] && ! kill -0 "${owner_pid}" 2>/dev/null; then
            ...
            die "Another JKAPI Apple container operation is already running."
        fi
    fi
    printf '%s\n' "$$" >"${LOCK_DIR}/pid"
    LOCK_ACQUIRED=true
...
cmd_init() {
    ...
    info "Review the file, then run: JKAPI_ENV_FILE='${ENV_FILE}' ${SCRIPT_DIR}/apple-container.sh up"
}
...
prepare_environment() {
    ...
    APP_IMAGE="$(read_env_value APPLE_CONTAINER_JKAPI_IMAGE weishaw/jkapi:latest)"
    ...
    POSTGRES_USER="$(read_env_value POSTGRES_USER jkapi)"
    POSTGRES_PASSWORD="$(read_env_value POSTGRES_PASSWORD)"
    POSTGRES_DB="$(read_env_value POSTGRES_DB jkapi)"
    ...
    TEMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/jkapi-apple.XXXXXX")"
...
create_app_container() {
    ...
    container create \
        ...
        --entrypoint /bin/sh \
        "${APP_IMAGE}" \
        -c 'set -e; mkdir -p "$DATA_DIR"; chown -R jkapi:jkapi "$DATA_DIR"; exec su-exec jkapi /app/jkapi' \
        >/dev/null
}
...
start_app() {
    ...
    if ! wait_for_probe "JKAPI" 180 probe_app; then
        show_failure_logs "${APP_CONTAINER}"
        die "JKAPI did not become ready."
    fi
    if ! wait_for_probe "JKAPI host port" 15 probe_host_app; then
...
cmd_up() {
    ...
    info "JKAPI is available at http://${ACCESS_HOST}:${HOST_PORT}"
}
...
cmd_down() {
    ...
    info "JKAPI stack stopped; persistent volumes were preserved."
}
...
cmd_logs() {
    ...
    case "${service}" in
        app|jkapi) container_name="${APP_CONTAINER}" ;;
...
confirm_destroy() {
    ...
    if [[ "${include_volumes}" == true ]]; then
        printf 'Delete the JKAPI stack and all persistent data? [y/N] '
    else
        printf 'Delete the JKAPI containers and network, preserving volumes? [y/N] '
    fi
    read -r answer
    [[ "${answer}" == "y" || "${answer}" == "Y" ]]
}
...
cmd_destroy() {
    ...
    if [[ "${include_volumes}" == true ]]; then
        ...
        info "JKAPI stack and persistent data deleted."
    else
        info "JKAPI stack deleted; persistent volumes were preserved."
    fi
}
