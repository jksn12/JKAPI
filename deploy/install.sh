#!/bin/bash
#
# JKAPI Installation Script
# JKAPI 安装脚本
# Usage: curl -sSL https://raw.githubusercontent.com/jksn12/JKAPI/main/deploy/install.sh | bash
#
...
# Configuration
GITHUB_REPO="jksn12/JKAPI"
INSTALL_DIR="/opt/jkapi"
SERVICE_NAME="jkapi"
SERVICE_USER="jkapi"
CONFIG_DIR="/etc/jkapi"
...
declare -A MSG_ZH=(
    ...
    # Completion
    ["install_complete"]="JKAPI 安装完成！"
    ["install_dir"]="安装目录"
    ["next_steps"]="后续步骤"
    ["step1_check_services"]="确保 PostgreSQL 和 Redis 正在运行："
    ["step2_start_service"]="启动 JKAPI 服务："
    ...
    ["not_installed"]="JKAPI 尚未安装，请先执行全新安装"
    ["fresh_install_hint"]="用法"

    # Uninstall
    ["uninstall_confirm"]="这将从系统中移除 JKAPI。"
    ...
    ["uninstall_complete"]="JKAPI 已卸载"

    # Help
    ["usage"]="用法"
    ["cmd_none"]="(无参数)"
    ["cmd_install"]="安装 JKAPI"
    ["cmd_upgrade"]="升级到最新版本"
    ["cmd_uninstall"]="卸载 JKAPI"
    ...
    ["server_config_desc"]="配置 JKAPI 服务监听地址"
...
declare -A MSG_EN=(
    ...
    # Installation
    ["install_title"]="JKAPI Installation Script"
    ...
    # Completion
    ["install_complete"]="JKAPI installation completed!"
    ["install_dir"]="Installation directory"
    ["next_steps"]="NEXT STEPS"
    ["step1_check_services"]="Make sure PostgreSQL and Redis are running:"
    ["step2_start_service"]="Start JKAPI service:"
    ...
    ["not_installed"]="JKAPI is not installed. Please run a fresh install first"
    ["fresh_install_hint"]="Usage"

    # Uninstall
    ["uninstall_confirm"]="This will remove JKAPI from your system."
    ...
    ["uninstall_complete"]="JKAPI has been uninstalled"

    # Help
    ["usage"]="Usage"
    ["cmd_none"]="(none)"
    ["cmd_install"]="Install JKAPI"
    ["cmd_upgrade"]="Upgrade to the latest version"
    ["cmd_uninstall"]="Remove JKAPI"
    ...
    ["server_config_desc"]="Configure JKAPI server listen address"
...
get_current_version() {
    if [ -f "$INSTALL_DIR/jkapi" ]; then
        # Use grep -E for better compatibility (works on macOS and Linux)
        "$INSTALL_DIR/jkapi" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' | head -1 || echo "unknown"
    else
        echo "not_installed"
    fi
}
...
download_and_extract() {
    ...
    local archive_name="jkapi_${version_num}_${OS}_${ARCH}.tar.gz"
    ...
    # Copy binary
    cp "$TEMP_DIR/jkapi" "$INSTALL_DIR/jkapi"
    chmod +x "$INSTALL_DIR/jkapi"
    ...
    print_success "$(msg 'binary_installed') $INSTALL_DIR/jkapi"
}
...
install_service() {
    ...
    cat > /etc/systemd/system/jkapi.service << EOF
[Unit]
Description=JKAPI - AI API Gateway Platform
Documentation=https://github.com/jksn12/JKAPI
...
User=jkapi
Group=jkapi
WorkingDirectory=/opt/jkapi
ExecStart=/opt/jkapi/jkapi
Restart=always
RestartSec=5
StandardOutput=journal
StandardError=journal
SyslogIdentifier=jkapi
...
enable_autostart() {
    ...
    if systemctl enable jkapi 2>/dev/null; then
...
print_completion() {
    ...
    echo "=============================================="
    echo ""
    echo "  $(msg 'cmd_status'):   sudo systemctl status jkapi"
    echo "  $(msg 'cmd_logs'):     sudo journalctl -u jkapi -f"
    echo "  $(msg 'cmd_restart'):  sudo systemctl restart jkapi"
    echo "  $(msg 'cmd_stop'):     sudo systemctl stop jkapi"
    echo ""
    echo "=============================================="
}
...
upgrade() {
    ...
    # Get current version
    CURRENT_VERSION=$("$INSTALL_DIR/jkapi" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' || echo "unknown")
    print_info "$(msg 'current_version'): $CURRENT_VERSION"

    # Stop service
    if systemctl is-active --quiet jkapi; then
        print_info "$(msg 'stopping_service')"
        systemctl stop jkapi
    fi

    # Backup current binary
    cp "$INSTALL_DIR/jkapi" "$INSTALL_DIR/jkapi.backup"
    print_info "$(msg 'backup_created'): $INSTALL_DIR/jkapi.backup"
    ...
    # Set permissions
    chown "$SERVICE_USER:$SERVICE_USER" "$INSTALL_DIR/jkapi"

    # Start service
    print_info "$(msg 'starting_service')"
    systemctl start jkapi

    print_success "$(msg 'upgrade_complete')"
}

# Install specific version (for upgrade or rollback)
# Requires: JKAPI must already be installed
install_version() {
    ...
    if [ -f "$INSTALL_DIR/jkapi" ]; then
        ...
        if [ "$current_version" != "unknown" ] && [ "$current_version" != "not_installed" ]; then
            backup_name="jkapi.backup.${current_version}"
        else
            backup_name="jkapi.backup.$(date +%Y%m%d%H%M%S)"
        fi
        cp "$INSTALL_DIR/jkapi" "$INSTALL_DIR/$backup_name"
...
uninstall() {
    ...
    print_info "$(msg 'stopping_service')"
    systemctl stop jkapi 2>/dev/null || true
    systemctl disable jkapi 2>/dev/null || true

    print_info "$(msg 'removing_files')"
    rm -f /etc/systemd/system/jkapi.service
    systemctl daemon-reload
