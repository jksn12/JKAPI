...
export XAI_API_KEY="${apiKey}"`
  ...
  // Shape follows Grok Build user guide (~/.grok/docs + custom-models) and production-ready JKAPI setups.
  ...
  const configContent = `# Grok Build CLI → JKAPI Grok group (API key auth).
# Docs: ~/.grok/docs/user-guide/05-configuration.md + 11-custom-models.md
# Verify after save: grok inspect
#
# IMPORTANT: api_backend must be "responses" for JKAPI Grok (POST /v1/responses).
...
# Prefer API key when using a custom gateway (matches JKAPI).
...
description = "Grok 4.5 via JKAPI (Responses)"
...
# Imagine tools: model IDs go to JKAPI media endpoints (not the text [model.*] catalog).
...
function generateGrokCodexFiles(baseUrl: string, apiKey: string): FileConfig[] {
  ...
  switch (shell) {
    case 'cmd':
      envPath = 'Command Prompt'
      envContent = `set JKAPI_API_KEY=${apiKey}`
      break
    case 'powershell':
    case 'windows':
      envPath = 'PowerShell'
      envContent = `$env:JKAPI_API_KEY="${apiKey}"`
      break
    default:
      envPath = 'Terminal'
      envContent = `export JKAPI_API_KEY="${apiKey}"`
  }

  const configContent = `# Codex CLI → JKAPI Grok group
...
# Grok/JKAPI path is HTTP/SSE; disable WS (Codex may otherwise try WebSocket first)
...
function generateOpenCodeConfig(platform: string, baseUrl: string, apiKey: string, pathLabel?: string): FileConfig {
  ...
  if (platform === 'gemini') {
    ...
    // Custom provider pointing at JKAPI OpenAI-compatible Responses/Chat endpoints.
    provider[platform].npm = '@ai-sdk/openai-compatible'
    provider[platform].name = 'Grok via JKAPI'
